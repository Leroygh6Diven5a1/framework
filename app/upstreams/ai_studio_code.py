...
def _convert_messages_to_contents(messages: list) -> tuple:
    ...
    for msg in messages:
            ...
            try:
                if isinstance(content, str):
                    ...
                    if stripped.startswith("{"):
                        ...
                        # 🛡️ 突破点 1：如果是 JSON 数组，必须包裹成字典！防谷歌 Protobuf 校验 invalid argument 崩溃
        ...
        # 2. 还原 Function Call 历史 (属于 model 角色)
            ...
            for tool_call in msg.tool_calls:
                ...
                # 🛡️ 严防 4：同时传递 camelCase 和 snake_case 确保完美兼容各种 GraphQL 解释器
                if thought_sig:
    ...
    # 🛡️ 突破点 3：严防 contents 为空导致谷歌直接爆 invalid argument 异常
    if not contents:
...
def _build_batch_graphql_body(project_id: str, model_name: str, request: OpenAIRequest) -> dict:
    ...
    # 🛡️ 突破点 2：识破生图/Imagen等非文本模型，绝对不注入思考配置，防 invalid argument 报错
    is_image_model = "image" in model_name.lower() or "imagen" in model_name.lower()
    
    if not is_image_model:
        # 动态适配思考配置，防 2.5 级等旧版模型因不识别 thinkingLevel 字段爆 invalid argument
